    ...
    return (
        <div className={`flex gap-4 ${isUser ? 'flex-row-reverse' : 'flex-row'} animate-in slide-in-from-bottom-2 duration-300`}>
            ...
            <div className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 ${isUser
                    ...
                    : 'bg-panel border border-gray-700 text-primary'
                }`}>
            ...
            <div className={`max-w-2xl flex flex-col gap-2 ${isUser ? 'items-end' : 'items-start'}`}>
                <div className={`px-5 py-3.5 rounded-2xl shadow-sm leading-relaxed ${isUser
                        ? 'bg-primary text-white rounded-tr-none'
                        : 'bg-panel border border-gray-700 text-gray-200 rounded-tl-none'
                    }`}>
                    {message.text}
                </div>
